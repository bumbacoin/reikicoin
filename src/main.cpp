...
const int64 nChainStartTime = 1385085544;
...
const string strMessageMagic = "reikicoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // reikicoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal reikicoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in reikicoin
    bool fStrictPayToScriptHash = true; // Always active in reikicoin
    ...
    // ppcoin: fees are not collected by miners as in reikicoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "reikicoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Do not worry and be filled with gratitude";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:reikicoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x6f915e68023a6cfe88a0e5bcf4c8dd27f5f7295a2cf632dcb62ba51fdc4e0889"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("reikicoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("reikicoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("reikicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("reikicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadreikicoinMiner(void* parg);

static bool fGeneratereikicoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void reikicoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("reikicoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratereikicoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratereikicoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running reikicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadreikicoinMiner(void* parg)
{
    ...
    try
    {
        ...
        reikicoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadreikicoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadreikicoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadreikicoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatereikicoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratereikicoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratereikicoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d reikicoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadreikicoinMiner, pwallet))
                printf("Error: NewThread(ThreadreikicoinMiner) failed\n");
            Sleep(10);
        }
    }
}
